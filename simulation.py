...
#            self._p = cp.concatenate([self._p, p])
#            self._d = cp.concatenate([self._d, d])
...
#        histogram_output.add(h/(bin_area * duration_s))
    ...
    def histogram(self, photon_batch, stage,
        ...
        #strides = 2 # 1.6 s
        #strides = 4 # 1.2 s =
        #strides = 8 # 1.1 s, 1.6s
        #strides = 16 # 1.1 s
        strides = 32 # 1.1 s
        #strides = 64 # 1.1 s, 1.1 s
        #strides = 128 # 1.1 s
        #strides = 256 # 1.1 s
        #strides = 512 # 1.2 s =
        #strides = 1024 # 1.2 s
...
#        bin_area_sr_m2 = (cp.cos(bin_edges[:-1]) - cp.cos(bin_edges[1:]) ) * 2 * np.pi * projected_area_m2
        Simulator.one_histogram_theta(grid_size, block_size, bins, size,
                photon_batch.alive,
                photon_batch.wavelength_nm,
                photon_batch.ez_z,
